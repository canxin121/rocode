<command-instruction>
You are starting an Atlas execution session from a Prometheus plan.
...
- `/start-work [plan-name] [--worktree <path>]`
  ...
  - `--worktree <path>` (optional): absolute path to an existing git worktree to work in
    - If specified and valid: worktree_path is pre-set in boulder.json
...
- The session_id is injected by the command runtime - use it directly
- Always update boulder.json BEFORE starting work
- Always set worktree_path in boulder.json before executing any tasks
- Read the FULL plan file before delegating any tasks
- Follow atlas delegation protocols
</command-instruction>
